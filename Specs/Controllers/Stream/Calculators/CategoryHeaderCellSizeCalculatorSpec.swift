////
///  CategoryHeaderCellSizeCalculatorSpec.swift
//
...
class CategoryHeaderCellSizeCalculatorSpec: QuickSpec {
    override func spec() {
        describe("CategoryHeaderCellSizeCalculator") {
            describe("should size proportionally according to frame width") {
                ...
                for (frameWidth, calcHeight) in expectations {
                    it("should size width \(frameWidth) to \(calcHeight)") {
                        let calculator = CategoryHeaderCellSizeCalculator()
